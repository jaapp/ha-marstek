#!/usr/bin/env python3
"""Network diagnostic test script for Marstek Local API integration.

Run this from your Home Assistant installation to test network connectivity.

Location when installed via HACS:
  /config/custom_components/marstek_local_api/test_network.py

Usage:
  python3 /config/custom_components/marstek_local_api/test_network.py

For debugging in Home Assistant, enable debug logging by adding to configuration.yaml:
...
# Get paths to integration modules (we're inside the integration directory now)
